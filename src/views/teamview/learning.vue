learn<template>
  <div id="TeamViewLearn" class="page-body g90">
    <div class="container">
      <Usernotice />
                ...
                <div class="bx--resource-card bx--resource-card--dark">
                  <div class="bx--tile">
                    <div class="bx--type-productive-heading-01 card-title padbot1">
                      Classes, by frequency (across entire team)
                    </div>
                    ...
                    <v-table :data="filteredItems" class="bx--data-table scrollable">
                      <thead slot="head" class="bx--data-table thead">
                        <tr class="bx--data-table">
                          ...
                          <v-th sortKey="classtitle" class="table-column classtitle">
                            Class Title
                          </v-th>
                          <v-th sortKey="start" class="table-column narrow">
                            Start
                          </v-th>
                          <v-th sortKey="end" class="table-column narrow">
                            End
                          </v-th>
                          <v-th sortKey="hours" class="table-column narrow">
                            Hours
                          </v-th>
                        </tr>
                      </thead>
                      <tbody slot="body" slot-scope="{displayData}" class="scrollbody">
                        <tr v-for="(row, index) in displayData" :key="index">
                          ...
                          <td class="table-column classtitle">
                            {{ row.classtitle }}
                          </td>
                          <td class="table-column start">
                            {{ row.start }}
                          </td>
                          <td class="table-column end">
                            {{ row.end }}
                          </td>
                          <td class="table-column hours">
                            {{ row.hours }}
                          </td>
                        </tr>
                      </tbody>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      filteredItems() {
        return this.learnTable.filter(item => {
          if (item.employee.toLowerCase().includes(this.search) || item.classtitle.toLowerCase().includes(this.search)) {
            return item;
          }
        })
      }
    ...
    methods: {
      getData() {
        axios.get('https://w3-services1.w3-969.ibm.com/myw3/unified-profile/v1/docs/instances/teamInfoResolved/' + this.manager).then((response) => {
          var quantity = response.data.doc.content.functional.reports.length.toString();
          this.minQuantity = Math.round(quantity * .2);
          this.maxQuantity = quantity;
          axios.get('https://site-' + this.shortenv + '-api.domain.net/api/teamview/learning/ZZZ/' + this.manager).then((response) => {
            this.learnTable = response.data.data.tablerows;
            this.namesAll = response.data.data.classNames;
      ...
      replaceNames() {
        var realnames = [...new Set(this.learnTable.map(learn => learn.employee))];
        for (var i = 0; i < this.learnTable.length; i++) {
          var a = realnames.indexOf(this.learnTable[i].employee);
          this.learnTable[i].employee = 'Employee #' + (a+1);
        }
      },
      newManager() {
