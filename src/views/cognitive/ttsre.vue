<template>
  <div class="container">
    <PageheadNoTabs title="Cognitive Talent Tapper - SRE" :origin="'ISA'" />
    <div v-if="!groupTeam && environment !== 'local'" class="nologin bx--type-body-short-01">
      <div v-if="!usertoken">
        You must login to view this page.
      </div>
      <div v-else>
        You are not authorized to view this page. If you believe this message is in error, please create
        an access request.
      </div>
    </div>
    <div v-else>
      <cv-tabs aria-label="navigation tab label">
        <cv-tab label="SRE Dashboard by Manager">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      loadFilters() {
        axios
          .post(process.env.ROOT_API + "/open/filters", {
            fields: [
              "yearEnrolled",
              "planTitle",
              "ISTSS",
