...
const apiEndPoint = 'https://saa.w3ibm.cloud.net/api';
...
// lockpoints for secure calls
// app.use('/api/lock', require('./routes/lockpoints')(app, samlConfig, passport));
// openpoints used for production endpoints
app.use('/api/open', require('./routes/openpoints')(app));
app.use('/api/prod/open', require('./routes/openpoints')(app));
// openpoint variations for non-production calls
app.use('/api/dev/open', require('./routes/nonprod')(app));
app.use('/api/staging/open', require('./routes/nonprod')(app));
app.use('/api/local/open', require('./routes/nonprod')(app));

// proxy api requests
const proxyTable = {
  '/api': {
    target: process.env.API_SERVER || 'https://site-dev-api.domain.net/api',
    changeOrigin: true,
    pathRewrite: {
      '^/api': ''
    }
...
app.post('/login/callback/postResponse', passport.patchResponse(), passport.authenticate(samlConfig.passport.strategy), function(req, res, next) {
    ...
    if (req.user.Groups.includes('ServiceAcademyAnalyticsCore')) {
      res.cookie('groupTeam', true, { expires: new Date(Date.now() + oneDay) });
    } else {
      res.cookie('groupTeam', false, { expires: new Date(Date.now() + oneDay) });
    };
    if (req.user.Groups.includes('ISA-Execs')) {
      res.cookie('groupExecs', true, { expires: new Date(Date.now() + oneDay) });
    } else {
      res.cookie('groupExecs', false, { expires: new Date(Date.now() + oneDay) });
    };
    if (req.user.Groups.includes('ISA-LKAdmin')) {
      res.cookie('groupAdmins', true, { expires: new Date(Date.now() + oneDay) });
    } else {
      res.cookie('groupAdmins', false, { expires: new Date(Date.now() + oneDay) });
    };
    if (req.user.Groups.includes('tableauLicensed')) {
      res.cookie('groupDash', true, { expires: new Date(Date.now() + oneDay) });
    } else {
      res.cookie('groupDash', false, { expires: new Date(Date.now() + oneDay) });
    };
